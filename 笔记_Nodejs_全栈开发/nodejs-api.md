...
* `mode`：文件类型与权限
    ...
    * `33204` 即 `100664`（八进制）或 `1000000110110100`（二进制）对应 `-rw-rw-r--`【不可执行的普通文件】

    * `33279` 即 `100777`（八进制）或 `1000000111111111`（二进制）对应 `-rwxrwxrwx`【可执行的高权限文件】

    * `8630` 即 `20666`（八进制）或 `0010000110110110`（二进制）对应 `crw-rw-rw-`

    * `25008` 即 `60660`（八进制）或 `0110000110110000`（二进制）对应 `brw-rw----`

    * `41471` 即 `120777`（八进制）或 `1010000111111111`（二进制）对应 `lrwxrwxrwx`【需要调用 `fs.lstat` 才能成功获取】

    * 以二进制形式分析，该值由两部分组成，前 7 位为文件类型，后 9 位（共分 3 组）为操作权限：

        1. `ll` 信息的第 1 位是，及 `mode` 信息前 7 位
        
            * `-`：`1000000`，文件
            * `d`：`0100000`，目录
            * `c`：`0010000`，character device
            * `b`：`0110000`，block device
            * `l`：`1010000`，链接
            * `p`：FIFO / named pipe
            * `s`：socket
